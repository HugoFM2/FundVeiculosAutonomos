...
def getInclination(imageMask,showImage=True):
	...
	if lines is not None:
		...
		cv2.line(image, (int(x_line), int(mask.shape[0])), (x2_car, y2_car), (0, 255, 0), 2)  # Desenha linha do carro
...
while car.t < 200:
	...
	# image = cv2.rectangle(image, (0,0), (w,h), (47,47,47), 40) 


	blank_image = np.zeros((h+10,w+10,3), np.uint8)
	...
	# top = int(h*0.5)
	top = int(h*0.8)
	image_without_top = image[0:h, 0:w]
	...
	delta,erro,psierro = ControleLateralVisao(car_x=300,line_x=rho,line_psi=theta,car_psi=0,k=0.7)
	# delta,erro,psierro = ControleLateralVisao(car_x=300,line_x=rho,line_psi=theta,car_psi=0,k=0.8)
	car.setSteer(delta) 
